...
export default {
    ...
    //显示一下示例数据
    showDwon() {
        window.demo.eventObj.markpoints(demoData);
    },
    ...
    loadEventDemoData(data) {
        window.demo.eventObj.markpoints(data);
    },
    showEventDemoData(data) {
    ...
    loadWarningTyphoonDemoData(data) {
    ...
    _createLayer(layerid) {
        let layer = LayerUtil.getLayerById(this.map, layerid);
        if (!layer) {
