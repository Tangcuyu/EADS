/**
 * Created by user on 2017/9/7.
 */
'use strict';
// 全局demo变量参数
window.demo = {};
// 初始一张图地图组件参数
window.demo.mapOptions = {
  'targetId': 'map', // ts-gis map需要的容器div id
  'defaultExtent': {
    'zoom': 5, // 初始级别
    'maxZoom': 20, // 最大级别
    'minZoom': 0, // 最小级别
    'center': [116.390833, 39.904832], // 中心点
    'projection': 'EPSG:4326'// 坐标系
  }
};
// 地图底图参数
window.demo.mapBaseOptions = [
  {
    'icon': '../../packages/theme-chalk/dist/images/switchmap/vector.png',
    'zIndex': -1,
    'attr': 'basics',

    'id': 'tiandituLayer_vec',
    'title': '天地图矢量',
    'type': 'group',
    'layers': [
      {
        'layers': 'vec',
        'matrixSet': 'c',
        'format': 'tiles',
        'tileType': 102,
        'crossOrigin': '*',
        // "url": "http://t0.tianditu.com/vec_c/wmts?",    //internet
        'url': 'http://t0.tianditu.gov.cn/vec_c/wmts?tk=4f62e1d82bd46e2ff470b291c2260156', // gsafety-yf
        // "url": "http://172.18.7.60/proxy/proxy/mapTiledProxy.ashx?http://t0.tianditu.cn/vec_c/wmts?", //gsafety-wuhan
        'projection': 'EPSG:4326'
      },
      {
        'layers': 'cta',
        'matrixSet': 'c',
        'format': 'tiles',
        'tileType': 102,
        'crossOrigin': '*',
        // "url": "http://t0.tianditu.com/cta_c/wmts?",//internet
        'url': 'http://t0.tianditu.gov.cn/cta_c/wmts?tk=4f62e1d82bd46e2ff470b291c2260156', // gsafety-yf
        // "url": "http://172.18.7.60/proxy/proxy/mapTiledProxy.ashx?http://t0.tianditu.cn/cta_c/wmts?",//gsafety-wuhan
        'projection': 'EPSG:4326'
      }
    ]
  }
] ;

// 台风警戒线参数
window.demo.typhoonAlertLineOptions = {
  id: 'typhoon_alertline',
  name: '台风警戒线',
  zIndex: 100,
  'pic24_xy': [127, 30], // 24小时警戒图片位置
  'pic48_xy': [132, 30], // 48小时警戒图片位置
  'lineStyle48h': 3, // 24小时警戒线样式 实线
  'lineStyle24h': 5, // 24小时警戒线样式 虚线
  'points_24h': [[126.993568, 34.005024], [126.993568, 21.971252 ], [118.995521, 17.96586], [118.995521, 10.97105], [113.018959, 4.48627], [104.998939, -0.035506]],
  'points_48h': [[131.981361, 33.959474], [131.981361, 14.96886 ], [119.962318, -0.035506], [104.998939, -0.035506]],
  'color_24h': '#FFFF00', // {r: 255, g: 255, b: 0}, //24小时警戒线颜色
  'color_48h': '#160DFF', //  {r: 255, g: 255, b: 0}, //data:image/png;base64,
  'pic_base24': 'data:image/png;base64,',
  'pic_base48': 'data:image/png;base64,'
} ;
// 台风标签参数
window.demo.typhoonComponentOptions = {

  map: null, // ts-gis map对象,必填
  simplyurl: '/deps/src/data/simply/', // 年台风数据集的服务路径,必填
  detailurl: '/deps/src/data/detail/', // 每条台风明细数据的服务路径,必填

  spatialReference: 4326,
  projection: 1, // 0：平面，1：球面
  // startYear:2015,//台风列表开始台风年

  tfbh: '201822', // 201409 //初始化显示的台风编号
  picSymbol: {// 台风风眼图片
    typhoonIcon: 'data:image/gif;base64,'
  },
  interfaceMsg: {
    'forecastPoint': { /* 鼠标移动获取台风预测点信息事件*/
      'name': 'mousemoveForecastPoint',
      'enable': 1
    },
    'trackPoint': { /* 点击获取台风真实轨迹点事件*/
      'name': 'clickTrackPoint',
      'enable': 1
    }
  },

  layerOrder: {
    trackLine: {
      id: 'typhoon_line',
      name: '台风路径线图层',
      zIndex: 101
    },
    trackPoint: {
      id: 'typhoon_point',
      name: '台风路径点图层',
      zIndex: 102
    },
    focusPoint: {
      id: 'typhoon_focus_point',
      name: '台风路径点焦点图层',
      zIndex: 101
    },
    trackLabel: {
      id: 'typhoon_label',
      name: '台风路径文本',
      zIndex: 100
    },
    typhoonRing: {
      id: 'typhoon_ring',
      name: '台风路径风圈',
      zIndex: 100
    },
    typhoonForecast: {
      id: 'typhoon_forecast',
      name: '预报图层',
      zIndex: 101
    },
    typhoonIcon: {
      id: 'typhoon_icon_Layer',
      name: '台风风眼图标',
      zIndex: 100
    }

  },
  typhoon: {
    lineColor: { a: 153, r: 255, g: 255, b: 255 }, // 线型颜色样式
    lineSingleColorFlag: 0, // 0:线型多色标志，缺省；1:线型单色颜色标志

    nameTextSymbol: {// 台风标注样式
      fontSize: 14,
      fontWeight: 'normal',
      fontFamilyName: '宋体',
      foreground: {a: 255, r: 0, g: 0, b: 0},
      borderColor: {a: 255, r: 0, g: 0, b: 0},
      offsetY: 20,
      borderThickness: 1
    },
    focusPointSymbol: {
      fillColor: '#FFFFFF', // 选中路径焦点样式
      fillOpacity: 153, // 153,255,
      borderColor: '#80808C', // #80808C
      opacity: 153,
      weight: 0.5,
      radius: 12
    },
    typhoonSpeeds: [[0.1, 10.7], [10.8, 17.1], [17.2, 24.4], [24.5, 32.6], [32.7, 41.4], [41.5, 50.9], [51, 120]],
    typhoonLevel: ['常态气压', '热带低压', '热带风暴', '强热带风暴', '台风', '强台风', '超强台风'],
    typhoonPointColor: ['#000000', '#02FF02', '#39C7CF', '#FBF9CC', '#FFDE75', '#FF8515', '#FF5252']
    // typhoonPointColor:['#000000','#02FF02','#0264FF', '#FFFB05',  '#FFAC05','#F171F9','#FE0202'],
  },
  forecast: {
    lineStyle: {
      opacity: 153, // 透明度
      width: 2, // 默认初始化为2
      style: 2// 简单直线样式,Dash = 0, DashDot = 1, DashDotDot = 2, Dot = 3, Null = 4,Solid = 5，默认初始化为2
    },
    org: {
      '中国': {
        rgb: '#FF0000',
        display: 1
      },
      // "CN":{   rgb: '#FF0000' ,  display:1 },
      '中国香港': {
        rgb: '#FFA500',
        display: 1
      },
      // "HK":{  rgb:'#FFA500' ,   display:1,  },
      '中国台湾': {
        rgb: '#FFFF00',
        display: 1
      },
      // "TW":{   rgb:'#FFFF00' ,  display:1,   },
      '日本': {
        rgb: '#00FF00',
        display: 1
      },
      // "JP":{ rgb:'#00FF00' , display:1,  },
      '美国': {
        rgb: '#0000FF',
        display: 1
      },
      // "US":{          rgb:'#0000FF',            display:1,        },
      '韩国': {
        rgb: '#B97A57',
        display: 1
      },
      // "KR":{            rgb:'#B97A57',            display:1,        },
      '南海': {
        rgb: '#800080',
        display: 1
      },
      // "NH":{            rgb:'#800080',            display:1,        },
      '浙江': {
        rgb: '#FFC0CB',
        display: 1
      },
      // "ZJ":{            rgb:'#FFC0CB',            display:1,        },
      '菲律宾': {
        rgb: '#22B14C',
        display: 1
      },
      '广东': {
        rgb: '#4B0082',
        display: 1
      },
      '欧洲': {
        rgb: '#B5E61D',
        display: 1
      },
      'null': {
        rgb: '#000000',
        display: 1
      }
    }
  },
  popup: {// 弹出框参数
    display: 1,
    offset: [-50, 10], // 偏移坐标x,y [-200,100]
    // div style
    divStyle: 'font-size:12px;color:#000 ;background: transparent; margin-top: 8px;margin-left: 10px; margin-right: 0px;',
    liStyle: 'height:20px;margin:2px;',
    pointEPSG: 4326, // point需要转换成的目标坐标系
    pointPrecision: 3, // point需要转换成的目标坐标小数位的精度
    // isShowRing : 0,//是否显示风圈.0展示。1不展示
    typhoonName: {
      display: 1,
      label: '台风编号：',
      unit: ''
    },
    passTime: {
      display: 1,
      label: '过去时间：',
      unit: ''
    },
    centerPoint: {
      display: 1,
      label: '中心位置：',
      x: '#lon#',
      y: '#lat#',
      format: '#lon#E,#lat#N',
      unit: ''
    },
    centerPressure: {
      display: 1,
      label: '中心气压：&nbsp;',
      unit: '&nbsp;百帕'
    },
    power: {
      display: 1,
      label: '最大风力：&nbsp;',
      unit: '&nbsp;级'
    },
    speed: {
      display: 1,
      label: '最大风速：&nbsp;',
      unit: '米/秒'
    },
    moveSpeed: {
      display: 1,
      label: '移动速度：&nbsp;',
      unit: '公里/小时'
    },
    moveDir: {
      display: 1,
      label: '移动方向：&nbsp;',
      unit: ''
    },
    sevenRingRadius: {
      display: 1,
      label: '七级风圈半径:',
      unit: '公里'
    },
    tenRingRadius: {
      display: 1,
      label: '十级风圈半径:',
      unit: '公里'
    },
    twelveRingRadius: {
      display: 1,
      label: '十二级风圈半径:',
      unit: '公里'
    }

  }, // OK

  popupForecast: {
    display: 1, // 显示控制
    offset: [-200, 100], // 偏移坐标x,y
    // div style
    divStyle: 'font-size:12px;color:#000 ;background: transparent; margin-top: 8px;margin-left: 10px; margin-right: 0px;',
    liStyle: 'height:20px;margin:2px;',
    pointEPSG: 4326, // point需要转换成的目标坐标系
    org: {
      display: 1,
      label: '预报机构：',
      unit: ''
    },
    arrivalTime: {
      display: 1,
      label: '到达时间：',
      unit: ''
    },
    centerPoint: {
      display: 1,
      label: '中心位置：',
      x: '#lon#',
      y: '#lat#',
      format: '#lon#,#lat#',
      unit: ''
    },
    power: {
      display: 1,
      label: '最大风力：&nbsp;',
      unit: '&nbsp;级'
    },
    speed: {
      display: 1,
      label: '最大风速：&nbsp;',
      unit: '米/秒'
    },
    moveSpeed: {
      display: 1,
      label: '移动速度：&nbsp;',
      unit: '公里/小时'
    },
    moveDir: {
      display: 1,
      label: '移动方向：&nbsp;',
      unit: ''
    },
    centralPressure: {
      display: 1,
      label: '中心气压：&nbsp;',
      unit: '&nbsp;百帕'
    }
    // pointPrecision:3 //point需要转换成的目标坐标小数位的精度
  }
};

// 用于切换的底图
window.demo.mapBaseSwitchOptions = [
  {
    'icon': '../../packages/theme-chalk/dist/images/switchmap/vector.png',
    'zIndex': -1,
    'attr': 'basics',

    'id': 'tiandituLayer_vec',
    'title': '天地图矢量',
    'type': 'group',
    'layers': [
      {
        'layers': 'vec',
        'matrixSet': 'c',
        'format': 'tiles',
        'tileType': 102,
        'crossOrigin': '*',
        // "url": "http://t0.tianditu.com/vec_c/wmts?",    //internet
        'url': 'http://t0.tianditu.gov.cn/vec_c/wmts?tk=4f62e1d82bd46e2ff470b291c2260156', // gsafety-yf
        // "url": "http://172.18.7.60/proxy/proxy/mapTiledProxy.ashx?http://t0.tianditu.cn/vec_c/wmts?", //gsafety-wuhan
        'projection': 'EPSG:4326'
      },
      {
        'layers': 'cta',
        'matrixSet': 'c',
        'format': 'tiles',
        'tileType': 102,
        'crossOrigin': '*',
        // "url": "http://t0.tianditu.com/cta_c/wmts?",//internet
        'url': 'http://t0.tianditu.gov.cn/cta_c/wmts?tk=4f62e1d82bd46e2ff470b291c2260156', // gsafety-yf
        // "url": "http://172.18.7.60/proxy/proxy/mapTiledProxy.ashx?http://t0.tianditu.cn/cta_c/wmts?",//gsafety-wuhan
        'projection': 'EPSG:4326'
      }
    ]
  },
  {
    'icon': '../../packages/theme-chalk/dist/images/switchmap/earth.png',
    'zIndex': -1,
    'attr': 'basics',

    'id': 'tiandituLayer_img',
    'title': '天地图影像',
    'type': 'group',
    'layers': [
      {
        'layers': 'img',
        'matrixSet': 'c',
        'format': 'tiles',
        'tileType': 102,
        // "url": "http://172.17.10.180:11012/img_c/wmts?",//internet
        // "url": "http://172.17.10.180:11012/img_c/wmts?",//gsafety-yf
        'url': 'http://t0.tianditu.gov.cn/img_c/wmts?tk=4f62e1d82bd46e2ff470b291c2260156', // gsafety-wuhan
        'projection': 'EPSG:4326'
      },
      {
        'layers': 'cta',
        'matrixSet': 'c',
        'format': 'tiles',
        'tileType': 102,
        // "url": "http://t0.tianditu.com/cta_c/wmts?",//internet
        // "url": "http://172.17.10.180:11012/cta_c/wmts?", //gsafety-yf
        'url': 'http://t0.tianditu.gov.cn/cta_c/wmts?tk=4f62e1d82bd46e2ff470b291c2260156', // gsafety-wuhan
        'projection': 'EPSG:4326'
      }
    ]
  }
];

